...
client.on('ready', async () => {
  console.log("ready comrade!")
  const guild = await client.guilds.fetch(process.env.SERVER);


  const channels = await guild.channels.fetch()
  const r = channels.get(process.env.CHANNEL)
  //getting channel and guild
   const task = new Task('simple task',async () => { await r.send('owo daily') })


   const job = new SimpleIntervalJob({ days:01, }, task)
   scheduler.addSimpleIntervalJob(job)
   async function dankSlash(){
     await r.send('owoh')
